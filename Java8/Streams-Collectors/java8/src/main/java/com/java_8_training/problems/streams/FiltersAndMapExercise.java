...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter(in -> in%2==0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(in -> in.equals(name)).collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(val -> val.stream().map(integer -> integer)).collect(toList());
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch(pers -> pers.getAge() <70);
        return overAge;
    }
    ...
    public static int findTheMinimumNumber(){
        ...
        min = input.reduce(999, Integer::min);
        return min;
    }
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(person -> person.getAge()).reduce(999,Integer::min);
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum += personList.stream().map(Person::getAge).reduce(0,Integer::sum);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(pers -> pers.getName().startsWith("J")).map(Person::getNationality).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(nr -> Math.sqrt(nr) == (int)(Math.sqrt(nr)) && nr %3 == 0).findFirst();
    ...
    public static int sumStringLength(){
        ...
        int sum = 0;
        //TODO #16
        sum = strings.stream().map(String::length).reduce(0,(acc,x) -> acc + x);
